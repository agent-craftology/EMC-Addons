...
public final class DungeonZoneScoreboard implements StatCardSource {
    ...
    public void deserialize(Properties p) {
        if (p == null) return;
        String zoneStage = p.getProperty("hud.dungeonzone.showZoneStage");
        showZoneStage = zoneStage == null || Boolean.parseBoolean(zoneStage);
        String respawn = p.getProperty("hud.dungeonzone.showRespawn");
    ...
    private void scanNearby(Minecraft client, long now) {
        AABB aabb = client.player.getBoundingBox().inflate(SCAN_RANGE);
        ...
        Map<Integer, MobLevelParser.Result> parsedByLevel = new HashMap<>();
    ...
    public boolean shouldSendQuery(Minecraft client, boolean masterVisible, boolean zoneCardVisible) {
