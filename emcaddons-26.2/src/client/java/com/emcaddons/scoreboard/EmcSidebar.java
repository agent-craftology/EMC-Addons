...
public final class EmcSidebar {
    ...
    public static Snapshot read(Minecraft client) {
        if (client == null || client.level == null) return Snapshot.empty();
    ...
    private static String reconstructLine(Scoreboard scoreboard, String ownerName) {
        ...
        try {
            for (String teamName : scoreboard.getTeamNames()) {
                ...
                if (team != null && team.getPlayers().contains(ownerName)) {
                    text = team.getPlayerPrefix().getString() + ownerName + team.getPlayerSuffix().getString();
                    break;
                }
            }
