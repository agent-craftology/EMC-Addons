...
public class ClickGuiScreen extends Screen {
    ...
    private static final Identifier LOGO = Identifier.fromNamespaceAndPath("emcaddons", "textures/gui/logo.png");
    ...
    private EmcStatsScoreboard.Gamemode selectedGamemode = EmcStatsScoreboard.Gamemode.DUNGEONS;
    ...
    public ClickGuiScreen(EmcAddonsClient mod) {
        super(Component.literal("EMC Addons"));
        this.mod = mod;
    ...
    private void rebuildRows() {
        ...
        switch (page) {
            ...
            case MODULE -> rows.addAll(ClickGuiPages.module(mod, this, selectedGamemode));
        }
        layoutRows();
    }
    ...
    void openModule(EmcStatsScoreboard.Gamemode mode) {
        if (mode != null) selectedGamemode = mode;
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        matrices.pushMatrix();
        matrices.translate(0, slide);
        drawRows(context, mouseX, mouseY - slide);
        matrices.popMatrix();
        GuiDraw.disableScissor(context);
        drawScrollbar(context, mouseX, mouseY);
        if (hoverTooltip != null) {
            GuiDraw.tooltip(context, font, hoverTooltip, mouseX, mouseY, width, height);
        }
        GuiScale.end(context);
    }

    private void drawScrollbar(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
    ...
    private void drawSidebar(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        ...
        GuiDraw.text(ctx, font, GuiTheme.PRODUCT, textX, textY, GuiTheme.TITLE);
        GuiDraw.text(ctx, font, modVersion, textX, textY + 11, GuiTheme.MUTED);
    ...
    private int drawNavSection(GuiGraphicsExtractor ctx, int mouseX, int mouseY, int startY, String title, boolean main) {
        GuiDraw.text(ctx, font, title, winX + 16, startY, GuiTheme.MUTED);
        int y = startY + 14;
        ...
        for (NavHit nav : navHits) {
            ...
            labelX = nav.x + 24;
            GuiDraw.text(ctx, font, nav.label, labelX, nav.y + 9, textColor);
            y += GuiTheme.NAV_H + 4;
        }
        return y;
    }
    ...
    private void drawHeader(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        ...
        switch (page) {
            ...
            case MODULE -> {
                title = selectedGamemode == null ? "Module" : selectedGamemode.displayName;
                sub = selectedGamemode == EmcStatsScoreboard.Gamemode.DUNGEONS
        ...
        if (isDrillIn()) {
            ...
            GuiDraw.textCentered(ctx, font, "Back", backX + backW / 2, backY + 7, GuiTheme.TITLE);
            tx = backX + backW + 10;
        }
        GuiDraw.text(ctx, font, title, tx, winY + 12, GuiTheme.TITLE);
        int titleW = font.width(title);
        GuiDraw.text(ctx, font, " | " + sub, tx + titleW, winY + 12, GuiTheme.MUTED);
        int themeRight = winX + winW - GuiTheme.PAD;
        if (showSearch()) {
            searchField.render(ctx, font, mouseX, mouseY);
            themeRight = winX + winW - GuiTheme.PAD - GuiTheme.SEARCH_W - 8;
        }
        themeSelector.layoutRight(themeRight, winY + 12, font);
        themeSelector.render(ctx, font, mouseX, mouseY);
    }

    private void drawRows(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        for (SettingRow row : rows) {
            row.render(ctx, font, mouseX, mouseY);
        }
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (searchField.isFocused() && searchField.keyPressed(event)) {
            return true;
        }
        for (SettingRow row : rows) {
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        if (searchField.isFocused() && searchField.charTyped(chr, 0)) return true;
        for (SettingRow row : rows) {
            if (row.charTyped(chr, 0)) return true;
        }
