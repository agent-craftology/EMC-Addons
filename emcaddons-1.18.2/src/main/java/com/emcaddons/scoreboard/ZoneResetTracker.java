...
public final class ZoneResetTracker {
    ...
    public boolean shouldSendQuery(boolean inDungeons, boolean hudAndCardVisible) {
        return shouldSendQuery(inDungeons, hudAndCardVisible, true, System.currentTimeMillis());
    }

    public boolean shouldSendQuery(boolean inDungeons, boolean hudAndCardVisible, long nowMs) {
        return shouldSendQuery(inDungeons, hudAndCardVisible, true, nowMs);
    }
    ...
    public boolean shouldSendQuery(boolean inDungeons, boolean hudAndCardVisible, boolean showRespawn, long nowMs) {
        if (!inDungeons || !hudAndCardVisible || !showRespawn) return false;
        expirePending(nowMs);
