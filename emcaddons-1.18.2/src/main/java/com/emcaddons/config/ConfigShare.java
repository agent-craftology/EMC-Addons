...
/**
 * Shareable {@code .cbshare} export/import for EMC Addons 1.18.2 profiles.
 *
...
public final class ConfigShare {
    ...
    private static volatile ConfigProfileManager boundProfiles;
    ...
    /** Point file-chooser defaults at this install's {@code config/emcaddons/exports}. */
    public static void bind(ConfigProfileManager manager) {
        boundProfiles = manager;
    }
    ...
    public static File defaultExportDir(File baseConfigDir) {
        ...
        File base = baseConfigDir != null ? baseConfigDir
                : (boundProfiles != null ? boundProfiles.getBaseConfigDir() : null);
        if (base != null) {
            File exports = new File(base, EXPORTS_DIR_NAME);
            exports.mkdirs();
    ...
    /**
     * Open a save dialog (default: {@code config/emcaddons/exports/<name>.cbshare}).
     * Result is applied on the client thread.
     */
    public static void chooseAndExport(ConfigProfileManager manager, String name, Consumer<String> chat) {
        if (manager == null) {
            tell(chat, "§cConfig is not ready");
            return;
        }
        if (name == null || !manager.profileExists(name)) {
            tell(chat, "§cProfile not found: " + name);
            return;
        }
        File startDir = defaultExportDir(manager.getBaseConfigDir());
        chooseExportFileAsync(name, startDir, picked -> {
            String err = exportPreset(manager, name, picked);
            if (err == null) {
                tell(chat, "§aExported profile '" + name + "' to " + ensureCbshareExtension(picked).getAbsolutePath());
            } else {
                tell(chat, "§c" + err);
            }
        }, () -> tell(chat, "§7Export cancelled."));
    }

    /** Open an open-file dialog for a {@code .cbshare} and import it. */
    public static void chooseAndImport(ConfigProfileManager manager, Consumer<String> chat) {
        if (manager == null) {
            tell(chat, "§cConfig is not ready");
            return;
        }
        File startDir = defaultExportDir(manager.getBaseConfigDir());
        chooseImportFileAsync(startDir, picked -> {
            String result = importPreset(manager, picked);
            if (isError(result)) {
                tell(chat, "§c" + errorMessage(result));
                return;
            }
            tell(chat, "§aImported profile '" + result + "'. Use /config load " + result + " to switch to it.");
        }, () -> tell(chat, "§7Import cancelled."));
    }

    private static void tell(Consumer<String> chat, String message) {
        if (chat == null) return;
        MinecraftClient mc = MinecraftClient.getInstance();
        if (mc != null) mc.execute(() -> chat.accept(message));
        else chat.accept(message);
    }
