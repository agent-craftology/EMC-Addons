...
/**
 * Shared palette and metrics (kept in lockstep with the 1.20.6 ClickGUI).
 */
public final class GuiTheme {
    ...
    public static int ROW_HOVER = 0xFF1E1F2A;
     ...
     * the slider. Text, accents and knobs stay fully opaque; HUD cards have
     * their own slider (see {@link #HUD_ALPHA_FLOOR_PCT}).
     */
    public static final int WINDOW_ALPHA_FLOOR_PCT = 40;
    /**
     * HUD card backgrounds are driven by their own slider. Each theme bakes its
     * own alpha into {@code HUD_BG}, but {@link #apply(Theme, int)} overrides the
     * alpha byte centrally so 100% is genuinely opaque on every theme (nothing
     * behind a card can bleed through) and 0% still floors at this percent so the
     * card stays legible.
     */
    public static final int HUD_ALPHA_FLOOR_PCT = 35;

    private static Theme current = Theme.EMERALD;
    private static int opacityPct = OPACITY_DEFAULT;
    private static int hudOpacityPct = OPACITY_DEFAULT;
    ...
    /** Re-paints the current theme with a new HUD card alpha. */
    public static void applyHudOpacity(int hudOpacityPercent) {
        hudOpacityPct = clampOpacity(hudOpacityPercent);
        apply(current, opacityPct);
    }

    /**
     * Keeps each theme's HUD RGB but replaces the baked alpha byte with the
     * slider value. Runs last in {@link #apply(Theme, int)} so neither a theme
     * switch nor the GUI opacity slider can clobber it.
     */
    private static void applyHudAlpha() {
        HUD_BG = (HUD_BG & 0x00FFFFFF) | (hudAlphaByte() << 24);
    }

    /** Slider 0–100 → HUD alpha 35–100% of fully opaque. */
    private static int hudAlphaByte() {
        float floor = HUD_ALPHA_FLOOR_PCT / 100f;
        float t = floor + (hudOpacityPct / 100f) * (1f - floor);
        int a = Math.round(255 * t);
