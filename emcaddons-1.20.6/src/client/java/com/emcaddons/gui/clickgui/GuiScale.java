...
public final class GuiScale {
    ...
    public static void begin(DrawContext ctx, int width, int height) {
        if (ctx == null) return;
        begin(ctx.getMatrices(), width, height);
    }

    public static void begin(MatrixStack matrices, int width, int height) {
        screenW = width;
        screenH = height;
        active = current();
        ...
        matrices.scale(active, active, 1f);
        matrices.translate(-width / 2.0, -height / 2.0, 0);
    }
    ...
    public static void end(MatrixStack matrices) {
        if (pushed && matrices != null) matrices.pop();
        pushed = false;
        active = 1f;
    }
