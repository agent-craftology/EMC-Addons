...
final class ClickGuiPages {
    ...
    static List<SettingRow> modules(ClickGuiScreen gui) {
        List<SettingRow> rows = new ArrayList<>();
        rows.add(new ModuleGridRow(
                new ModuleGridRow.Card("Dungeons", "Track dungeon currencies, rates, and grind time",
                        () -> gui.openModule(ClickGuiScreen.GameMode.DUNGEONS)),
                new ModuleGridRow.Card("Gens", "Coming Soon!", () -> gui.openModule(ClickGuiScreen.GameMode.GENS)),
                new ModuleGridRow.Card("Factories", "Coming Soon!", () -> gui.openModule(ClickGuiScreen.GameMode.FACTORIES)),
                new ModuleGridRow.Card("Skyblock", "Coming Soon!", () -> gui.openModule(ClickGuiScreen.GameMode.SKYBLOCK)),
                new ModuleGridRow.Card("Prisons", "Coming Soon!", () -> gui.openModule(ClickGuiScreen.GameMode.PRISONS))
        ));
        return rows;
    }

    static List<SettingRow> module(EmcAddonsClient mod, ClickGuiScreen gui) {
        List<SettingRow> rows = new ArrayList<>();
        ClickGuiScreen.GameMode mode = gui.selectedModule();
        EmcStatsScoreboard sb = mod.getEmcStatsScoreboard();
        rows.add(new HeadingRow(mode.title.toUpperCase(Locale.ROOT)));
        if (mode.comingSoon) {
            rows.add(new LabelRow("Status", () -> "Coming Soon!"));
            rows.add(new ButtonRow("Reset statistics", "Reset", () -> sb.resetModeBucket(mode.id)));
        } else {
            rows.addAll(dungeonHudControls(mod, gui));
            rows.add(new ButtonRow("Reset statistics", "Reset", sb::resetSession));
        }
        return rows;
    }
