...
- HUD cards for **Souls**, **Essence**, **Shards**, **Credits**, **Money**, **Swings**, **Rebirth**, **Grind Time**, plus a sparkline **Graph**, and a **Zone** card that shows Zone and Stage from nearby dungeon mob custom names and floating text displays, plus remaining mob respawn time from `/zone reset` (auto-queried, hidden)
...
- **Zone** — a separate card for Zone and Stage inferred from nearby dungeon mob custom names and floating text displays (for example `[RARE] LVL1 Chicken`), plus remaining mob respawn time from `/zone reset` (auto-queried, hidden)
